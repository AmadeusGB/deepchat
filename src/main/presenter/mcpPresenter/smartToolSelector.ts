...
export class SmartToolSelector {
  ...
  private readonly BROWSER_KEYWORDS = [
    '浏览器', 'browser', '网页', 'webpage', '访问网站', 'visit', '点击', 'click', 
    'playwright', '截图', 'screenshot', '导航', 'navigate', '页面', 'page',
    '元素', 'element', '输入文本', 'type', '选择选项', 'select', '上传文件', 'upload',
    '等待', 'wait', '悬停', 'hover', '拖拽', 'drag', '键盘', 'key',
    '网络请求', 'network', '控制台', 'console', '标签页', 'tab', '窗口', 'window',
    'mcp_playwright', 'browser_', '打开网址', 'open url', '网站登录', 'web login',
    '表单填写', 'fill form', '页面操作', 'page action', '浏览器操作', 'browser action',
    '登录', 'login', '访问', 'access', '网页登录', 'web access', '打开', 'open',
    '网站访问', 'site visit', '页面访问', 'page visit', '浏览', 'browse', '上网', 'surf web'
  ]
  
  private readonly DEVICE_KEYWORDS = [
    'deeper设备', 'deeper device', 'dpn隧道', 'dpn tunnel', 'dpn路由', 'dpn routing',
    '家长控制', 'parental control', '广告拦截', 'ads filter', '内容过滤', 'content filter',
    '应用重定向', 'app relocator', 'ssl bypass', 'https过滤', 'https filter', 'dns过滤',
    '恶意软件拦截', 'malware block', '追踪拦截', 'tracker block', '设备重启', 'device reboot',
    '设备配置', 'device config', '设备地址', 'device url', '设备连接', 'device connect',
    'dpn模式', 'dpn mode', '智能路由', 'smart routing', '完全路由', 'full routing', '直接路由', 'direct routing',
    'loginToDeeperDevice', 'setDpnMode', 'getDpnMode', 'listTunnels', 'addTunnel',
    'setParentalControl', 'setAdsFilter', 'setSslBypass', 'rebootDevice', 'setBaseUrl',
    'listApps', 'setAppTunnelCode', 'deeper api', 'deeper 接口'
  ]
  
  // 高优先级浏览器操作关键词
  private readonly HIGH_PRIORITY_BROWSER_KEYWORDS = [
    'mcp_playwright', 'playwright', 'browser_', '浏览器操作', 'browser action',
    '网页操作', 'web operation', '页面操作', 'page operation'
  ]
  
  // 高优先级设备操作关键词  
  private readonly HIGH_PRIORITY_DEVICE_KEYWORDS = [
    'deeper设备', 'deeper device', 'dpn模式', 'dpn mode',
    'loginToDeeperDevice', 'setDpnMode', 'getDpnMode', 'listTunnels', 'addTunnel',
    'setParentalControl', 'setAdsFilter', 'setSslBypass', 'rebootDevice',
    'deeper api', 'deeper 接口', '设备api', 'device api'
  ]
  ...
  public async findBestTool(
    ...
    // 1. 精确匹配检查
    const exactMatch = availableTools.find(tool => 
      tool.function.name === requestedTool
    )
    ...
    const candidates = availableTools.map(tool => {
      ...
      const toolIntent = this.analyzeIntent(
        tool.function.name, 
    ...
    console.log(`[SmartToolSelector] 最佳匹配: "${bestMatch.tool}" (评分: ${bestMatch.score.toFixed(3)})`)
