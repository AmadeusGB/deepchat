...
const mockToolDefinitions: MCPToolDefinition[] = [
  ...
  // Deeper Device 工具
  {
    ...
    function: {
      ...
      description: '🔐 登录Deeper设备管理界面 - 使用admin/admin默认凭证或自定义用户名密码获取访问权限',
      parameters: { type: 'object', properties: {}, required: [] }
    },
  ...
  },
  {
    ...
    function: {
      name: 'setDpnMode',
      description: '🚀 配置DPN路由模式 - 选择Smart Routing(智能路由)、Full Routing(完全路由)或Direct Routing(直接路由)',
      parameters: { type: 'object', properties: {}, required: [] }
    },
    server: { name: 'deeper-device', icons: '🌐', description: 'Deeper device management' }
  },
...
const testCases = [
  ...
  { input: 'browser_navigate', expected: 'mcp_playwright_browser_navigate', description: '浏览器导航' },
  ...
  { input: 'playwright', expected: 'mcp_playwright_browser_navigate', description: 'Playwright操作' },
