      ...
      <div class="flex flex-col p-2 gap-2 px-2">
        <div class="flex flex-row items-center gap-2">
          <span class="flex flex-row items-center gap-2 flex-grow w-full">
            ...
            <span class="text-sm font-medium">{{ t('settings.voice.oppositeGenderResponse') }}</span>
          </span>
        ...
        <div class="flex flex-row items-center gap-2">
          ...
          <div class="flex-shrink-0">
            ...
            >
              ...
              {{ isTestingVoice ? t('settings.voice.testVoicePlaying') : t('settings.voice.testVoicePlay') }}
            </Button>
          </div>
        </div>
        <div class="pl-6 text-xs text-muted-foreground">
...
const genderDetectionStatus = computed(() => {
  ...
  const genderText = result.gender === 'male' ? t('settings.voice.genderDetectionMale') : t('settings.voice.genderDetectionFemale')
...
const genderDetectionDetails = computed(() => {
  ...
  return t('settings.voice.genderDetectionDetailsWithResult', { confidence: (result.confidence * 100).toFixed(1) })
})
...
const handleOppositeGenderResponseChange = async (enabled: boolean) => {
  try {
    ...
    toast({
      ...
      description: enabled ? t('settings.voice.oppositeGenderEnabled') : t('settings.voice.oppositeGenderDisabled'),
    })
  } catch (error) {
...
const handleUserGenderChange = async (gender: string) => {
  try {
    ...
    const genderText = gender === 'auto' ? t('settings.voice.userGenderAuto') : (gender === 'male' ? t('settings.voice.userGenderMale') : t('settings.voice.userGenderFemale'))
    toast({
      title: t('settings.voice.settingsSaved'),
...
const testVoiceResponse = async () => {
  ...
  try {
    ...
    if (result.gender === 'male') {
      testText = '您好！我是您的AI助手小深。我检测到您是男性用户，所以我用女声为您服务。希望您喜欢这种交互方式！'
    } else if (result.gender === 'female') {
      testText = '您好！我是您的AI助手小深。我检测到您是女性用户，所以我用男声为您服务。希望您喜欢这种交互方式！'
    } else {
      testText = '您好！我是您的AI助手小深。目前还没有检测到您的性别信息，请先使用语音功能让我了解您的声音特征。'
    }
